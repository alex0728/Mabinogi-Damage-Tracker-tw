...
function DamageLabels({ largestDamageInstance }) {
    ...
    // Get the player identifier (name or ID)
    const playerLabel = largestDamageInstance?.player_name || String(largestDamageInstance?.player_id || '');

    return (
        <div>
            {(!largestDamageInstance) ? null : lineSeries.map((series, index) => {
                if (series.label === playerLabel)
...
function DPSBands({ bands }) {
    ...
    return (
        <g>
            {(!bands || !Array.isArray(bands)) ? null : bands.map((b, index) => {
                if (!b || b.start === undefined || b.end === undefined) return null;
                const playerLabel = b?.player_name || String(b?.player_id || '');
                const matchingSeries = lineSeries.find((series) => series.label === playerLabel);
                const color = matchingSeries?.color || '#888';
