...
export default function RecordingsMenu() {
    ...
    const columns = [
        ...
        { field: 'skillStats', headerName: t('recordings.skillStats'), width: 80, flex: 0, renderCell: (params) => (
            <Tooltip title={t('skillStats.title')} disableInteractive>
                <IconButton
                    variant="text"
                    color="secondary"
                    onClick={() => {
                        setMenu({ name: "SkillStats", props: { start_ut: params.row.start_ut, end_ut: params.row.end_ut } })
                    }}
                >
                    <WhatshotIcon />
                </IconButton>
