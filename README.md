...
본 프로젝트의 개발자들은 익명을 희망하는 사람도 있지만, 모두 마비노기의 열정적인 플레이어들입니다. 저는 마비노기의 플레이어 경험을 향상시키며 많은 MMO RPG의 기능 수준에 맞도록 이 애플리케이션을 제작했습니다.\
...
1. 下載 Release 版本
2. 解壓縮
3. 執行 `Mabinogi_Damage_Tracker.Server.exe`
4. 開啟瀏覽器訪問 `http://localhost:5004`
...
**後端**
```bash
dotnet publish Mabinogi_Damage_Tracker.Server/Mabinogi_Damage_Tracker.Server.csproj -c Release -r win-x64
```
