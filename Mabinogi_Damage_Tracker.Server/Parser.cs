...
namespace Mabinogi_Damage_tracker
{
    public static class Parser
    {
        ...
        #endif
        #if !DEBUG_LIVE && !RELEASE && !DEBUG_FILE
            static LibPcapLiveDevice device = null;
        #endif
        ...
        private static void pack_damage(TcpPacket tcp, int cursor, int sub_packet_length, int begining_of_packet_cursor)
        {
            ...
            try
            {
                ...
                SkillId skill = 0;
                SkillId subskill = 0;
                UInt16 current_skill_id = 0; // Store current skill ID for enemy damage packets
                ...
                for (int i = 0; i < subsub_packet_count; i++)
                {
                    ...
                    // DEBUG: Test all offsets 1-50 for skill ID and log to frontend
                    string skillid_offsets = "";
                    for (int offset = 1; offset <= 50; offset++)
                    {
                        int test_cursor = cursor + offset;
                        if (test_cursor + 1 < tcp.PayloadData.Length)
                        {
                            UInt16 test_skillid = BinaryPrimitives.ReadUInt16BigEndian(tcp.PayloadData.AsSpan(test_cursor));
                            if (test_skillid == 22001 || test_skillid == 23002 || test_skillid == 20001 || test_skillid == 20020)
                            {
                                skillid_offsets += $"***FOUND +{offset}:" + test_skillid + "*** ";
                            }
                            else
                            {
                                skillid_offsets += $"+{offset}:" + test_skillid + " ";
                            }
                        }
                    }
                    ...
                    // Use original skillid (works for both KR and TW servers)
                    UInt16 final_skillid = skillid;

                    if ((subsub_ttype & 2) != 0)
                    {
                        attacker_id = entityID;
                        
                        skill = (SkillId)final_skillid;
                        current_skill_id = final_skillid; // Store for enemy damage packets
                        subskill = (SkillId)subskillid;

                        // DEBUG: Log skill parsing to console
                        // Console.WriteLine($"[DEBUG v4] Attack - Type:0x{subsub_ttype:X2}, SkillID:{(UInt16)skill}");
                    ...
                    if ((subsub_ttype & 1) != 0)
                    {
                        ...
                        if(damage < 0 || damage > 100000000 || (UInt16)skill == 601 || (UInt16)skill == 512 || (UInt16)skill == 590) { break; }

                        // Console.WriteLine($"[DAMAGE v4] Attacker: 0x{attacker_id:X} -> Enemy: 0x{enemy_id:X} for {damage} with SkillID:{current_skill_id}");
                        LogsController.WriteLog(string.Format("[DAMAGE] Attacker: {0} -> Enemy: {1} for {2}", attacker_id, enemy_id, damage));
                        db_helper.add_damage((Int64)attacker_id, damage, wound, (int)manaDamage, (Int64)enemy_id, current_skill_id, subskillid);
                    }
