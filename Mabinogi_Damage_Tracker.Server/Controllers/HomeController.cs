...
namespace Mabinogi_Damage_tracker.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        /// <summary>
        /// Get skill statistics (skill usage count, avg/min/max damage)
        /// </summary>
        public ActionResult GetSkillStats(int start_ut, int end_ut, long? playerid = null)
        {
            List<SkillStat> skillStats = db_helper.Get_Skill_Stats(start_ut, end_ut, playerid);
            if (skillStats == null) { return NotFound(); }
            return Json(skillStats);
        }

        /// <summary>
        /// Get combat duration statistics
        /// </summary>
        public ActionResult GetCombatStats(int start_ut, int end_ut)
        {
            CombatStats combatStats = db_helper.Get_Combat_Stats(start_ut, end_ut);
            if (combatStats == null) { return NotFound(); }
            return Json(combatStats);
        }

        /// <summary>
        /// Get unique players from damages
        /// </summary>
        public ActionResult GetPlayers(int start_ut, int end_ut)
        {
            List<PlayerInfo> players = db_helper.Get_Players(start_ut, end_ut);
            if (players == null) { return NotFound(); }
            return Json(players);
        }

        /// <summary>
        /// Get player DPS stats
        /// </summary>
        public ActionResult GetPlayerDps(int start_ut, int end_ut)
        {
            List<PlayerDps> playerDps = db_helper.Get_PlayerDps(start_ut, end_ut);
            if (playerDps == null) { return NotFound(); }
            return Json(playerDps);
        }
